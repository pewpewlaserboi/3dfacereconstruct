...
class MyMainWindow(QMainWindow, Ui_MainWindow):
    ...
    def ProcessClicked(self):
        ...
        self.ui.Tslider.setRange(S * Root['Tex']['WTex'][0], S * Root['Tex']['WTex'][1])
        ...
        self.ui.Gslider.setRange(Root['models']['WGeo'][0], Root['models']['WGeo'][1])
    ...
    def PCA_Tex(self):
        ...
        ## >>> ADD CODE BELOW <<<
        numImages = 2
        sample = imread('model1.png')
        imsize = sample.shape
        images = [np.float32(imread('model1.png')/255), np.float32(imread('model2.png')/255)]

        #calculate the mean vector
        data = np.zeros((numImages, imsize[0]*imsize[1]*imsize[2]),dtype=np.float32)
    ...
    def PCA_Geo(self):
        ...
        ## >>> ADD CODE BELOW <<<
        model1 = OBJFastV('model1.obj')
        model2 = OBJFastV('model2.obj')
        numObjects = 2
        sample = np.array(model1.vertices)
        objsize = sample.shape
        objects = [np.array(model1.vertices), np.array(model2.vertices)]

        data = np.zeros((numObjects, objsize[0]*objsize[1]), dtype = np.float32)
        for i in range(0, numObjects):
            obj = objects[i].flatten()
            data[i,:] = obj
    ...
    def T_SliderValueChange(self, value):
        ...
        if b_ProcessDone == True and b_Process2Done == True:
            ...
            TarTexture = np.reshape(N_TarTex, (256, 256, 4))
    ...
    def G_SliderValueChange(self, value):
        global Gval, Root, TarModel, b_ProcessDone, b_Process2Done, P_Gval, Gval
        Gval = value
        ...
        if b_ProcessDone == True and b_Process2Done == True:
            ...
            ## Reshape the variable N_TarModel to have the real geometry format : x y z
            ...
            count = 0
            ## Create 3 for loops to reconstruct completely
            ## The first 5904 values in N_TarModel are X, next 5904 are Y, last 5904 are Z


            for x in range(0,len(N_TarModel),3):
                arr_3d[count][0] = N_TarModel[x]
                count += 1

            count = 0

            for y in range(1,len(N_TarModel),3):
                arr_3d[count][1] = N_TarModel[y]
                count += 1

            count = 0

            for z in range(2,len(N_TarModel),3):
                arr_3d[count][2] = N_TarModel[z]
                count += 1
            ...
            TarModel.vertices = tmp  # TarModel.vertices is the new 3D model
